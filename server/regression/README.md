...
- Local TF structure blocks LONG/SHORT path
- HTF structure blocks LONG/SHORT path
...
| G5-a: TF swing high close to entry | Rejected with `Entry too close to resistance` | Setup / `OK` | Real behavior gap. Fixed by requiring 1.5 ATR of local structure clearance before nearby support/resistance. |
| G5-b: HTF swing high close to entry | Rejected with `Entry too close to HTF resistance` | Setup / `OK` | Real behavior gap. Fixed by requiring 2.0 ATR of HTF structure clearance before nearby support/resistance. |
...
The two stale reason-text failures were updated in `server/test-refactor.ts`. The TP1 freshness, HTF conflict, weak zone rejection, local structure clearance, and HTF structure clearance verdict mismatches were fixed.
